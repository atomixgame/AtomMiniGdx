...
public abstract class AbstractSteeringBehaviour extends Behaviour {
     ...
     * @return
     */
    protected abstract Vector3f calculateSteeredVelocity();
     ...
     */
    protected float getAgentTotalMass() {
        float mass = 0;
        mass += agent.getMass();
        return mass;
    }
    ...
    protected void controlUpdate(float tpf) {
        this.velocity = calculateSteeredVelocity().mult(tpf);
        agent.getLocalTranslation().addLocal(this.velocity);
        rotateAgent(tpf);
    }
}
