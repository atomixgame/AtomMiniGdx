...
public class Road3D extends Node {
    ...
    protected Road2D road2D;
    protected Node roadLineNode;
    protected Node roadLineDebugNode;
    ...
    /**
     * A Road3D can be decomposed into Road3D segment.
     *
     * <p>A Segment made up by four points
     */
    public class Road3DSegment {
    ...
    public void createRoad(Vector3f start, Vector3f end) {
    ...
    public void randomRoad() {
        ArrayList<Vector3f> points = makeRandomRoadLine(new Vector3f(0, 0, 0), new Vector3f(15, 0, 4));
        setPathPoints(points);
    }

    public void setPathPoints(ArrayList<Vector3f> points) {
        Path newPath = new Path(points);
        road2D = new Road2D(newPath);
    }

    public void updateVisual() {
        //road2D.setPath(newPath);
        roadLineNode = new Node("Roadline Nodes");
        //FIXME: Get point from road2d!
        ArrayList<Vector3f> points = road2D.getPath().getPoints();
        displayRoadLine(points);
        attachChild(roadLineNode);
        Mesh mesh = makeRoadMesh(true);
        //makeRoadMeshSpline(points);
        makeRoadGeometry(mesh);
        createDebug();
    }

    public void makeRoadGeometry(Mesh roadMesh) {
    ...
    /**
     * Create a Mesh out of Road segments.
     *
     * <p>Replace this with SurfaceMesh.
     *
     * @param points
     * @param smooth
     * @return
     */
    public Mesh makeRoadMesh(boolean smooth) {
        ...
        //Ask if the segment is last
        //Ask if the segment denoted with cross road
...
//                    System.out.println(" Sign u1 " + su1 + " u1: " + u1);
                ...
                if (i < numOfPatches - 1) {
                    ...
                    //FIXME: Should be much more shorter!!!!
                    // Intersect 1
...
//            System.out.println("------------------------------");
    ...
    public float signVec(Vector3f v1, Vector3f v2, Vector3f v3, Vector3f v4) {
        return FastMath.cos(v2.subtract(v1).angleBetween(v4.subtract(v3)));
    }

    protected void alignQuad(Vector3f p1, Vector3f p2, float angle, float roadWidth, Vector3f rp0, Vector3f rp1, Vector3f rp2, Vector3f rp3) {
    ...
    protected void addQuad(Vector3f[] vertices, Vector2f[] texCoord, float[] indexes, float[] normals, int i, float l, Vector3f rp0, Vector3f rp1, Vector3f rp2, Vector3f rp3) {
        int vi = 0, ti = 0, ii = 0, ni = 0;
        vi = i * 4;
        ti = i * 4;
        ii = i * 6;
...
//    }
    public void attachProp(Spatial prop) {
    }

    public void travelEachSegment() {
    }
}
