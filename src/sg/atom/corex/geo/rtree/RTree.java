 ...
 **/
package sg.atom.corex.geo.rtree;
...
import sg.atom.core.datastructure.collection.TOrderedList;
import sg.atom.core.datastructure.collection.TPriorityQueue;
import sg.atom.corex.geo.PointFloat;
import sg.atom.corex.geo.Rectanglef;
import sg.atom.corex.geo.SpatialIndex2D;
...
public class RTree implements SpatialIndex2D, Serializable {
    ...
    private TPriorityQueue distanceQueue =
            new TPriorityQueue(TPriorityQueue.SORT_ORDER_ASCENDING);
     ...
     */
    public void add(Rectanglef r, int id) {
     ...
     */
    public boolean delete(Rectanglef r, int id) {
        ...
        while (foundIndex == -1 && parents.size() > 0) {
            ...
            if (!n.isLeaf()) {
                ...
                for (int i = startIndex; i < n.entryCount; i++) {
                    if (Rectanglef.contains(n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i], n.entriesMaxY[i],
     ...
     */
    public void nearest(PointFloat p, TIntProcedure v, float furthestDistance) {
    ...
    private void createNearestNDistanceQueue(PointFloat p, int count, float furthestDistance) {
        distanceQueue.reset();
        distanceQueue.setSortOrder(TPriorityQueue.SORT_ORDER_DESCENDING);
        ...
        while (parents.size() > 0) {
            ...
            if (!n.isLeaf()) {
                ...
                boolean near = false;
                for (int i = startIndex; i < n.entryCount; i++) {
                    if (Rectanglef.distanceSq(n.entriesMinX[i], n.entriesMinY[i],
                ...
                for (int i = 0; i < n.entryCount; i++) {
                    float entryDistanceSq = Rectanglef.distanceSq(n.entriesMinX[i], n.entriesMinY[i],
                            n.entriesMaxX[i], n.entriesMaxY[i],
                            p.x, p.y);
                    int entryId = n.ids[i];
     ...
     */
    public void nearestNUnsorted(PointFloat p, TIntProcedure v, int count, float furthestDistance) {
     ...
     * float)
     */
    public void nearestN(PointFloat p, TIntProcedure v, int count, float furthestDistance) {
        createNearestNDistanceQueue(p, count, furthestDistance);

        distanceQueue.setSortOrder(TPriorityQueue.SORT_ORDER_ASCENDING);
     ...
     * (ie less than 10).
     */
    public void nearestN_orig(PointFloat p, TIntProcedure v, int count, float furthestDistance) {
        // return immediately if given an invalid "count" parameter
        if (count <= 0) {
            return;
        }
     ...
     */
    public void intersects(Rectanglef r, TIntProcedure v) {
        RTreeNode rootNode = getNode(rootNodeId);
     ...
     */
    public void contains(Rectanglef r, TIntProcedure v) {
        ...
        while (parents.size() > 0) {
            ...
            if (!n.isLeaf()) {
                ...
                for (int i = startIndex; i < n.entryCount; i++) {
                    if (Rectanglef.intersects(r.minX, r.minY, r.maxX, r.maxY,
                ...
                for (int i = 0; i < n.entryCount; i++) {
                    if (Rectanglef.contains(r.minX, r.minY, r.maxX, r.maxY,
                            n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i], n.entriesMaxY[i])) {
    ...
    private RTreeNode splitNode(RTreeNode n, float newRectMinX, float newRectMinY, float newRectMaxX, float newRectMaxY, int newId) {
        ...
        if (INTERNAL_CONSISTENCY_CHECKING) {
            Rectanglef nMBR = new Rectanglef(n.mbrMinX, n.mbrMinY, n.mbrMaxX, n.mbrMaxY);
            if (!nMBR.equals(calculateMBR(n))) {
                log.error("Error: splitNode old node MBR wrong");
            }
            Rectanglef newNodeMBR = new Rectanglef(newNode.mbrMinX, newNode.mbrMinY, newNode.mbrMaxX, newNode.mbrMaxY);
        ...
        // debug code
        if (log.isDebugEnabled()) {
            float newArea = Rectanglef.area(n.mbrMinX, n.mbrMinY, n.mbrMaxX, n.mbrMaxY)
                    + Rectanglef.area(newNode.mbrMinX, newNode.mbrMinY, newNode.mbrMaxX, newNode.mbrMaxY);
    ...
    private int pickNext(RTreeNode n, RTreeNode newNode) {
        ...
        for (int i = 0; i < maxNodeEntries; i++) {
            if (entryStatus[i] == ENTRY_STATUS_UNASSIGNED) {
                ...
                float nIncrease = Rectanglef.enlargement(n.mbrMinX, n.mbrMinY, n.mbrMaxX, n.mbrMaxY,
                        n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i], n.entriesMaxY[i]);
                float newNodeIncrease = Rectanglef.enlargement(newNode.mbrMinX, newNode.mbrMinY, newNode.mbrMaxX, newNode.mbrMaxY,
                        n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i], n.entriesMaxY[i]);
                ...
                if (difference > maxDifference) {
                    ...
                    if (nIncrease < newNodeIncrease) {
                        ...
                        nextGroup = 1;
                    } else if (Rectanglef.area(n.mbrMinX, n.mbrMinY, n.mbrMaxX, n.mbrMaxY) < Rectanglef.area(newNode.mbrMinX, newNode.mbrMinY, newNode.mbrMaxX, newNode.mbrMaxY)) {
                        nextGroup = 0;
                    } else if (Rectanglef.area(newNode.mbrMinX, newNode.mbrMinY, newNode.mbrMaxX, newNode.mbrMaxY) < Rectanglef.area(n.mbrMinX, n.mbrMinY, n.mbrMaxX, n.mbrMaxY)) {
                        nextGroup = 1;
     ...
     */
    private float nearest(PointFloat p, RTreeNode n, float furthestDistanceSq) {
        for (int i = 0; i < n.entryCount; i++) {
            float tempDistanceSq = Rectanglef.distanceSq(n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i], n.entriesMaxY[i], p.x, p.y);
     ...
     */
    private boolean intersects(Rectanglef r, TIntProcedure v, RTreeNode n) {
        for (int i = 0; i < n.entryCount; i++) {
            if (Rectanglef.intersects(r.minX, r.minY, r.maxX, r.maxY, n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i], n.entriesMaxY[i])) {
                if (n.isLeaf()) {
                    if (!v.execute(n.ids[i])) {
                        return false;
                    }
    ...
    private RTreeNode chooseNode(float minX, float minY, float maxX, float maxY, int level) {
        ...
        while (true) {
            ...
            float leastEnlargement = Rectanglef.enlargement(n.entriesMinX[0], n.entriesMinY[0], n.entriesMaxX[0], n.entriesMaxY[0],
                    minX, minY, maxX, maxY);
            ...
            for (int i = 1; i < n.entryCount; i++) {
                ...
                float tempEnlargement = Rectanglef.enlargement(tempMinX, tempMinY, tempMaxX, tempMaxY,
                        minX, minY, maxX, maxY);
                if ((tempEnlargement < leastEnlargement)
                        || ((tempEnlargement == leastEnlargement)
                        && (Rectanglef.area(tempMinX, tempMinY, tempMaxX, tempMaxY)
                        < Rectanglef.area(n.entriesMinX[index], n.entriesMinY[index], n.entriesMaxX[index], n.entriesMaxY[index])))) {
                    index = i;
    ...
    private boolean checkConsistency(int nodeId, int expectedLevel, Rectanglef expectedMBR) {
        ...
        Rectanglef calculatedMBR = calculateMBR(n);
        Rectanglef actualMBR = new Rectanglef();
        ...
        for (int i = 0; i < n.entryCount; i++) {
            ...
            if (n.level > 1) { // if not a leaf
                if (!checkConsistency(n.ids[i], n.level - 1, new Rectanglef(n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i], n.entriesMaxY[i]))) {
                    return false;
                }
            }
        }
     ...
     */
    private Rectanglef calculateMBR(RTreeNode n) {
        Rectanglef mbr = new Rectanglef();
