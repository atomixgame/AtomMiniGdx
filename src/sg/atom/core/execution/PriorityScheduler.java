...
import sg.atom.core.execution.PriorityScheduler.PrioritySchedulableRecord;

/**
 * A {@code PriorityScheduler} works like a {@link LoadBalancingScheduler} but
 * allows different tasks to get a different share of the available time by
 * assigning a priority to each task. The higher the priority the longer the
 * amount of the available time dedicated to the corresponding task.
 *
 * @author davebaol
 */
public class PriorityScheduler extends SchedulerBase<PrioritySchedulableRecord> {
     ...
     *
     * @param dryRunFrames number of frames simulated by the dry run to
     * calculate the phase when adding a schedulable via
     * {@link #addWithAutomaticPhasing(Schedulable, int)} and
     * {@link #addWithAutomaticPhasing(Schedulable, int, float)}
     */
    ...
    /**
     * Executes scheduled tasks based on their frequency and phase. This method
     * must be called once per frame.
     *
     * @param timeToRun the maximum time in nanoseconds this scheduler should
     * run on the current frame.
     */
    @Override
    ...
    /**
     * Adds the {@code schedulable} to the list using the given
     * {@code frequency}, priority 1 and a phase calculated by a dry run of the
     * scheduler.
     *
    ...
    /**
     * Adds the {@code schedulable} to the list using the given
     * {@code frequency} and {@code priority} while the phase is calculated by a
    ...
    /**
     * Adds the {@code schedulable} to the list using the given
     * {@code frequency} and {@code phase} with priority 1.
     *
     * @param schedulable the task to schedule
     * @param frequency the frequency
    ...
    /**
     * Adds the {@code schedulable} to the list using the given
     * {@code frequency}, {@code phase} and priority.
     *
     * @param schedulable the task to schedule
     * @param frequency the frequency
     * @param phase the phase
